...
server.post('/register', (request, response) => {
    ...
    if(!userName){
        request.session.errors.push('User name was not provided');
    }
    const password = request.body['password'];
    if(!password){
        request.session.errors.push('Password was not provided');
    }
    //hash Password
    const passwd =
...
server.get('/entry/:entryID/comment/:id/update', (request, response) => {
    ...
    let entryId = request.params['entryID'];
...
server.post('/comment/:id/delete', (request, response) => {
    if (!request.session.authorized) {
        response.status(401).end('Unauthorized');
    ...
    if (request.path.endsWith('/delete')) {
        Comment.destroy({
            ...
            request.session.errors.push('Failed to remove the entry comment.');
            response.redirect('/entries');
        });  
        response.redirect('/'); 
        return; 
